...
# -*- coding: utf-8 -*-
"""
Created on Tue May  4 00:13:53 2021
...
def hammer(data):
    ...
    # vedo se la prima è rossa e la seconda verde
    if cdstk_2['open'] < cdstk_2['close']:
        # engulfing
        if 3 * (cdstk_2['close'] - cdstk_2['open']) < (cdstk_2['high'] - cdstk_2['low']):
            
             if cdstk_2['high'] < cdstk_2['close'] + cdstk_2['close'] * 0.05:
                print('hammer')
                return True
             else:
                print('no hammer')
...
class cspattern (Thread):
    ...
    def __init__(self, nome, bot, user):
        ...
        self.symbol_list = ['BATBTC','ETHBTC']		
        self.interval = '1h'
        self.limit = '1000'			
        ...
        while True: 
            for symbol in self.symbol_list:
                ...
                if bullish_engulfing(new):
                    self.bot.send_message(self.user,"Si è verificata una Bullish Engulfing su "+str(symbol))
                    
                if hammer(new):
                    self.bot.send_message(self.user,"Si è verificata un Hammer su "+str(symbol))    
